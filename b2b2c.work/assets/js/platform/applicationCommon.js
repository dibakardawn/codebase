...
const SITENAME = 'B2B2C';
const PROJECTPATH = 'https://b2b2c.work/';
...
const INACTIVITYTIMEOUT = 5; // 5 minutes
...
let	  INACTIVITYTIMER;
const financeCategoryORDS = 4; //earningType ORDS
const financeCategoryORDP = 11; //expenseType ORDP


/*---------------------Set the localForage driver for ADMIN only--------------------------*/
...
const appCommonFunctionality = (function (window, $) {
	...
	parent.getCurrentDatetime = function () { 
		const date = new Date();
	...
	parent.ajaxFormDataUpload = function(qryStr, formData, callback, showLoader = true) {
		$.ajax({
			type: "POST",
			url: APIPATH + qryStr,
			data: formData,
			...
			error: function(xhr, status, error) {
				console.error(`AJAX FormData Error: ${error}`, xhr);
				parent.hideLoader();
			}
		});
	};
	...
	parent.getDefaultData = function(identifire){
		...
		switch (identifire) {
			...
			case "COUNTRY":
				return {
					country: projectInformationData?.defaultInformation?.country || 'United Kingdom (UK)',
					countryId: projectInformationData?.defaultInformation?.countryId || '39'
				};
				
			case "CURRENCY":
				return {
					currency: projectInformationData?.defaultInformation?.currency || 'USD',
					currencySign: projectInformationData?.defaultInformation?.currencySign || '$'
				};
				
			case "TAX":
				return projectInformationData?.defaultInformation?.tax || 0;
	...
	parent.cmsImplementationThroughID = function() {
		...
		$("[id^=cms_]").each(function(e) {
			...
			var originalText = this.innerText; // Store original text
	...
	parent.formatDate = function(dateStr){
		const options = { year: 'numeric', month: 'short', day: '2-digit' };
		const date = new Date(dateStr);
		return date.toLocaleDateString('en-GB', options).replace(/ /g, '-');
	};
	
	parent.resetInactivityTimer = function(){
		clearTimeout(INACTIVITYTIMER);
		INACTIVITYTIMER = setTimeout(parent.logoutDueToInactivity, (INACTIVITYTIMEOUT * 60 * 1000));
	};
	
	parent.logoutDueToInactivity = function(data){
